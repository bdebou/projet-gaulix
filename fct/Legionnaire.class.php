...
class Legionnaire{
	...
	public function CombatLegionnaire(personnage &$Joueur){
		...
		if($ValeurJoueur > $this->ValeurCombat){		//Le joueur gagne le combat contre le Légionnaire
			$Joueur->UpdatePoints($lstPoints['CombatGagné']);
			$Joueur->UpdateScores(1, 0);
			$txtMessage = '<p>Vous avez gagné le combat contre le légionnaire romain. Vous avez gagné '.abs($lstPoints['CombatGagné']).'pts et 5pts d\'expérience.</p>';
			...
			$Joueur->UpdatePoints($lstPoints['CombatPerdu']);
			$Joueur->UpdateScores(0, 1);
			$Joueur->PerdreVie(($this->ValeurCombat - $arDefenseJoueur[0] - $arDefenseJoueur[1]), 'legionnaire');
			$txtMessage = '<p>Vous avez perdu le combat contre le légionnaire romain. Vous avez donc perdu '.abs($lstPoints['CombatPerdu']).'pts et '.($this->ValeurCombat - $arDefenseJoueur[0] - $arDefenseJoueur[1]).'pts '.AfficheIcone('vie').' mais quand même gagné 5 pts d\'expérience.</p>';
		}
