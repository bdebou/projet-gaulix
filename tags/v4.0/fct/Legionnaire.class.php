...
class Legionnaire{
	...
	public function CombatLegionnaire(personnage &$Joueur){
		...
		if($ValeurJoueur > $this->ValeurCombat){		//Le joueur gagne le combat contre le Légionnaire
			...
			$txtMessage = '<p>Vous avez gagné le combat contre le légionnaire romain. Vous avez gagné '.abs(personnage::POINT_COMBAT).'pts et 5pts d\'expérience.</p>';
			...
			$txtMessage = '<p>Vous avez perdu le combat contre le légionnaire romain. Vous avez donc perdu '.abs(personnage::POINT_COMBAT).'pts et '.($this->ValeurCombat - $arDefenseJoueur[0] - $arDefenseJoueur[1]).'pts '.AfficheIcone('vie').' mais quand même gagné 5 pts d\'expérience.</p>';
		}
